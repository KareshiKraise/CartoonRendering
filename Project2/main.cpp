...
void initshaders(void) {
   ...
   // my_program.enableTess(true);
	...
	{
		my_program.loadShader("hatching.vertex", "hatching.frag");
	}
...
void render()
{
...

	{
		...
		GLuint uniID = glGetUniformLocation(my_program.program, "LightColor");
		glUniform3fv(uniID, 1, glm::value_ptr(LightColor));
		...
		GLuint oMatrixID = glGetUniformLocation(my_program.program, "NormalMatrix");
		glUniformMatrix3fv(oMatrixID, 1, GL_FALSE, &NormalMatrix[0][0]);
		
		
		 oMatrixID = glGetUniformLocation(my_program.program, "M");
		glUniformMatrix4fv(oMatrixID, 1, GL_FALSE, &Model[0][0]);

		oMatrixID = glGetUniformLocation(my_program.program, "V");
		glUniformMatrix4fv(oMatrixID, 1, GL_FALSE, &Camera[0][0]);

		oMatrixID = glGetUniformLocation(my_program.program, "P");
		glUniformMatrix4fv(oMatrixID, 1, GL_FALSE, &Projection[0][0]);
		...
		uniID = glGetUniformLocation(my_program.program, "tex1");
		glUniform1i(uniID, 1);

		uniID = glGetUniformLocation(my_program.program, "tex2");
		glUniform1i(uniID, 2);
	
		uniID = glGetUniformLocation(my_program.program, "tex3");
		glUniform1i(uniID, 3);

		uniID = glGetUniformLocation(my_program.program, "tex4");
		glUniform1i(uniID, 4);

		uniID = glGetUniformLocation(my_program.program, "tex5");
		glUniform1i(uniID, 5);
...
int main(int argc, char **argv) {
	...
	printf("Cell shading - 0 ; Hatching - 1");
	...
	{	
		...
		myModel.loadfile("Kirby/bola.obj");
		myModel.loadtex("Kirby/hatching/hatch1.png");
		myModel.loadtex("Kirby/hatching/hatch2.png");
		myModel.loadtex("Kirby/hatching/hatch3.png");
		myModel.loadtex("Kirby/hatching/hatch4.png");
		myModel.loadtex("Kirby/hatching/hatch5.png");
		myModel.loadtex("Kirby/hatching/hatch6.png");

		//myModelloadNormalMap("Kirby/TesteHatch/stripes2.png");
		//myModel.loadHeigthDepthMap("Kirby/TesteHatch/stripes3.png");
	}
